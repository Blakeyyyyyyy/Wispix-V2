...
import { TaskManagerChat } from './TaskManagerChat';
...
export const TaskManagerDashboard: React.FC<TaskManagerDashboardProps> = ({ className = '' }) => {
  ...
  const handleRunAgent = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:3001/api/dev/run/TaskManager', { method: 'POST' });
      ...
      if (result.ok) {
        ...
        alert(`Failed to run TaskManager: ${result.error || 'Unknown error'}`);
        setIsLoading(false);
      }
    } catch (error) {
