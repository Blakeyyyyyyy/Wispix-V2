...
interface CredentialFormProps {
  ...
  threadId: string;
  userId: string;
  onCredentialSubmitted?: (message: string) => void;
}
...
export function CredentialForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Save each credential field individually
      for (const { platform, credentialName, requestedFields } of platforms) {
        for (const field of requestedFields) {
          const credentialValue = credentials[credentialName]?.[field];
          if (credentialValue?.trim()) {
            console.log(`CredentialForm - Saving ${field} for ${platform}`);
            
            // Create a safe service name for database storage
            const safeFieldName = field.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
            const serviceName = `${platform.toLowerCase()}_${safeFieldName}`;
            ...
            const credentialData = {
              ...
              credentials: {
                ...
                platform: platform,
                credential_name: field // Store exact field name from agent
              }
            };
            
            console.log('CredentialForm - Supabase payload:', credentialData);
            ...
            console.log('CredentialForm - Supabase response data:', data);
            console.log('CredentialForm - Supabase response error:', error);
            
            if (error) {
              console.error(`CredentialForm - Failed to save ${field} for ${platform}:`, error);
              setError(`Failed to save ${field} for ${platform}: ${error.message}`);
              setSaving(false);
              return;
            }
            
            console.log(`CredentialForm - Successfully saved ${field} for ${platform} with ID: ${data?.id}`);
          }
        }
      }
  ...
  const getFieldConfig = (field: string) => {
    ...
    return {
      label: field, // Use exact field name from agent
      type: inputType,
