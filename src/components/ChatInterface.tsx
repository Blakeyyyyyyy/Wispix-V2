...
export function ChatInterface({ thread }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    // Listen for credential submission messages from CredentialForm
    const handleCredentialMessage = (event: CustomEvent) => {
      const { message } = event.detail;
      if (message) {
        // Send the message through the normal sendMessage flow
        sendMessageDirectly(message);
      }
    };
    
    // Add event listener to the chat interface element
    const chatElement = document.querySelector('[data-chat-interface]');
    if (chatElement) {
      chatElement.addEventListener('sendCredentialMessage', handleCredentialMessage as EventListener);
    }
    ...
    return () => {
      ...
      const chatElement = document.querySelector('[data-chat-interface]');
      if (chatElement) {
        chatElement.removeEventListener('sendCredentialMessage', handleCredentialMessage as EventListener);
      }
  ...
  const parseMessageContent = (message: ChatMessage) => {
    try {
      ...
      if (parsed.success && parsed.message) {
        ...
        // If this is a flow change, trigger flow update
        if (hasFlowChange) {
          console.log('🔄 Detected flow change from Agent1');
